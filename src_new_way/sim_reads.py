...
class SimReads:
    ...
    def sim_reads_genome(self) -> tuple[str, str]:
        chrs = np.array([len(fasta.seq) for fasta in SeqIO.parse(open(self.fasta_file), "fasta")])
    ...
    def sim_reads_with_InSilicoSeq(self, N: int) -> tuple[str, str]:
        ...
        os.system(command)
        return f"{self.pathout}{self.cov}_R1.fastq", f"{self.pathout}{self.cov}_R2.fastq"
