...
def test_generate_coords(cnv_generator, mocker, chr_data):
    ...
    result = cnv_generator._CNVGenerator__generate_coords(chr_data)
...
def test_find_CNV_windows(cnv_generator, mocker, chr_data):
    ...
    mocker.patch.object(random, "randrange", side_effect=[1, 51, 101])
    lengths = [4, 5, 6]

    starts, ends = cnv_generator._CNVGenerator__find_CNV_windows(chr_data, lengths)
    assert len(starts) == len(ends)
    assert starts == [1, 51, 101]  # Mocked start value is used for all CNVs
    assert ends == [5, 56, 107]  # Stop coordinates are calculated based on lengths
...
@pytest.mark.parametrize(
    "starts, ends, start, stop, expected",
    [
        ([100, 200, 300], [150, 250, 350], 359, 400, False),
        ([100, 200, 300], [150, 250, 350], 360, 450, True),
        ([100, 200, 400], [150, 250, 450], 0, 91, False),
        ([100, 200], [150, 250], 0, 90, True),
        ([], [], 100, 200, True),
        ([100], [200], 150, 250, False),
        ([100, 400], [300, 600], 200, 300, False),
        ([200, 300, 400], [250, 350, 450], 100, 150, True),
        ([100], [200], 50, 150, False),
        ([100], [200], 50, 120, False),
    ],
)
def test_check_overlapping(cnv_generator, starts, ends, start, stop, expected):
    result = cnv_generator._CNVGenerator__check_overlaping(start, stop, starts, ends)
...
def test_max_cnv_length_too_large(cnv_generator):
    ...
    result = cnv_generator._CNVGenerator__max_cnv_lenght_too_large(len_fasta=1000)
    assert result is False

    # Test when len_fasta is equal to max_cnv_length
    result = cnv_generator._CNVGenerator__max_cnv_lenght_too_large(len_fasta=75)
    assert result is True

    # Test when len_fasta is less than max_cnv_length
    result = cnv_generator._CNVGenerator__max_cnv_lenght_too_large(len_fasta=50)
    assert result is True

    # Test with negative values
    result = cnv_generator._CNVGenerator__max_cnv_lenght_too_large(len_fasta=-100)
    assert result is True

    # Test with zero values
    result = cnv_generator._CNVGenerator__max_cnv_lenght_too_large(len_fasta=0)
